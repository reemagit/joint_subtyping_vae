...
def visualize_eltree_with_data(tree_elpi,X,X_original,color,variable_names,principal_component_vectors=None,mean_vector=None,
                              ...
                              add_color_bar=False,
                              vmin=None,vmax=None,
                              percentile_contraction=20,order=None):
    ...
    # Added from Enrico Maiorino
    if order is not None:
        my_idx = np.argsort(color2) if order == 'ascending' else np.argsort(color2)[::-1]
        plt.scatter(x[my_idx],y[my_idx],c=color2[my_idx],cmap=cmap,s=points_size[my_idx], vmin=vmin, vmax=vmax,alpha=Transparency_Alpha_points)
    else:
...
# Functions added by Enrico Maiorino
# This is a quick and dirty way to get the plot coordinates for the graph without plotting it
def return_eltree_coords(tree_elpi, embeds, labels, scatter_parameter=0.03):

    final_coords = {}

    X = embeds.values
    X_original = embeds.values
    color='k'
    variable_names = embeds.columns.tolist()
    Color_by_partitioning=True
    visualize_partition=labels
    showBranchNumbers=True

    principal_component_vectors=None
    mean_vector=None
    ...
    Transparency_Alpha_points = 1
    verbose=False
    Visualize_Branch_Class_Associations = []
    cmap = 'cool'
    ...
    final_coords['x_data'] = x
    final_coords['y_data'] = y
    ...
    tree_elpi['NodePositions2D'] = idx
    #plt.scatter(idx[:,0],idx[:,1],s=node_size,c='black',alpha=.8)
    final_coords['x_nodes'] = idx[:,0]
    final_coords['y_nodes'] = idx[:,1]
    ...
    final_coords['edges'] = []
    final_coords['edges_lbl'] = []
    final_coords['branches_lbl'] = []
    final_coords['nodes_lbl'] = []
    #Plot edges
    for j in range(len(edges)):
        x_coo = np.concatenate((idx[edges[j,0],[0]],idx[edges[j,1],[0]]))
        y_coo = np.concatenate((idx[edges[j,0],[1]],idx[edges[j,1],[1]]))

        final_coords['edges'].append((x_coo,y_coo))
        final_coords['edges_lbl'].append(((x_coo[0]+x_coo[1])/2,(y_coo[0]+y_coo[1])/2, j))
    ...
    if showBranchNumbers:
        ...
        for i,val in enumerate(branch_vals):
            ...
            final_coords['branches_lbl'].append((xbm,ybm,int(val)))
    if showNodeNumbers:
        for i in range(nodep.shape[0]):
            #plt.text(idx[i,0],idx[i,1],str(i),fontsize=20,bbox=dict(facecolor='grey', alpha=0.5))
            final_coords['nodes_lbl'].append((idx[i,0],idx[i,1],str(i)))
