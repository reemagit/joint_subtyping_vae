...
data_dir = Path('data/pheno')
branches_dir = Path('gendata')
out_dir = Path('gendata/prospective')
...
pheno_p3 = pd.read_table(data_dir / 'raw/COPDGene_P1P2P3_Flat_SM_NS_Mar20.txt',index_col=0)
pheno_p3 = pheno_p3.loc[sids]

mortality = pd.read_table(data_dir / 'processed/vital_status.tsv', index_col=0)
mortality = mortality.loc[sids]

#sm_sidlevel_raw = pd.read_table(data_dir / 'raw/LFU_SidLevel_Comorbid_SM_31AUG22.txt',index_col=0)

sm_surveylevel_raw = pd.read_table(data_dir / 'raw/LFU_Survey_Level_SM_NS_31AUG22.txt')
...
#cph_matrix['packyears'] = pheno_p3.ATS_PackYears_P2  # We remove this because is contained in the branch
