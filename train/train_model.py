...
@click.option('-s', '--seed', 'seed', type=int, default=None)
...
def main(input_dir, output_dir, mode, n_iter, zdim, bs, h1, h2, p_drop, mmd_weight, alpha_12, alpha_numcat, alpha_l1, alpha_l2, lr, seed, restore_dir, experiment):
	...
	if restore_dir:
		...
		model_state = torch.load(restore_dir / 'model.pth')
		vae = CoupledVAE.deserialize(model_state)

		optimizer = optim.Adam(vae.parameters())
		optimizer_state = torch.load(restore_dir / 'optimizer.pth')

		splits_df = loader_funcs.load_splits(restore_dir / "splits_df.tsv")


		optimizer.load_state_dict(optimizer_state)

		logger.info(f'Restoring session {restore_dir}:')
		logger.info(f'Input data dir: {input_dir}')
		logger.info(f'Output data dir: {output_dir}')
		logger.info(f'Experiment: {experiment}')
		for param, val in model_state['hyperparams'].items():
			if param == 'vars_meta':
				continue
			logger.info(param + ': ' + str(val))
		logger.info('Learning rate: {}'.format(optimizer_state['param_groups'][0]['lr']))
		...
		cdm = CoupledDatasetModule(input_dir, bs=bs)
		cdm.setup()
		cdm.split(test_size=0.1, validation_size=0.1, random_state=seed, scaler='minmax')
	...
	embeds.to_csv(output_dir / 'embeddings.tsv', sep='\t')
